module terarec{
    ...
    proc tera_compare(key_a: [1..10] uint(8), key_b: [1..10] uint(8)){
        for i in 1..10{
            if key_a[i] > key_b[i]{
                return 1;
    ...
    proc kv_tera_compare(ref keys, idx_a: int(64), idx_b: int(64)){
        var retval: int(8) = 0;
        for i in 1..10 {
            if ((keys[idx_a, i] : int(8)) > (keys[idx_b, i] : int(8))){
                retval = 1;
                break;
            } else if ((keys[idx_a, i] : int(8)) < (keys[idx_b, i] : int(8))){
                retval = -1;
                break;
            }
        }
    ...
    proc kv_ref_tera_compare(ref key_a, ref key_b){
        ...
        for i in 1..10 {
            if ((key_a[i] : int(8)) > (key_b[i] : int(8))){
                retval = 1;
                break;
            } else if ((key_a[i] : int(8)) < (key_b[i] : int(8))) {
                retval = -1;
                break;
            }
        }
