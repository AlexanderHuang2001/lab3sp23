...
proc main() {
    ...
    var start_time: real(64);
    var end_time: real(64);
    ...
    if here.id == 0 && timing {
        start_time = getCurrentTime(unit=TimeUnits.seconds);
    }

    coforall loc in Locales {
        on loc do {
            forall i in keys.localSubdomain().dim(0).first..keys.localSubdomain().dim(0).last {
                for j in 1..10 {
    ...
    if here.id == 0 && timing {
        end_time = getCurrentTime(unit=TimeUnits.seconds);
        var elapsed_time: real(64) = end_time - start_time;
        writeln("Locale 0: elapsed time for first backup copy is:", elapsed_time);
    }

    for run_num in 1..runcount {

        var metric_start_time: real(64) = getCurrentTime(unit=TimeUnits.seconds);

        terasort(keys = keys_ref, values = values_ref, keys_out = keys_out_ref, values_out = values_out_ref, counts_out = counts_out_ref);

        var metric_end_time: real(64) = getCurrentTime(unit=TimeUnits.seconds);
        var metric_elapsed_time: real(64) = metric_end_time - metric_start_time;

        if here.id == 0 && timing {
            writeln("Locale 0: metric elapsed time is ", metric_elapsed_time);
            start_time = getCurrentTime(unit=TimeUnits.seconds);
        }
        ...
        if here.id == 0 && timing {
            end_time = getCurrentTime(unit=TimeUnits.seconds);
            var elapsed_time: real(64) = end_time - start_time;
            writeln("Locale 0: elapsed time for teravalidate is:", elapsed_time);
        }

        runtimes[run_num] = metric_elapsed_time;
        ...
        if here.id == 0 && timing {
            end_time = getCurrentTime(unit=TimeUnits.seconds);
            var elapsed_time: real(64) = end_time - start_time;
            writeln("Locale 0: elapsed time for interior local copy is:", elapsed_time);
        }

        for count in counts_out {
            count = 0;
        }
