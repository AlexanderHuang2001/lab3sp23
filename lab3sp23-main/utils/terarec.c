...
terarec_t* teraGenerate(int  len){
	...
	for(int  i = 0; i < len; i++){
		...
		assignRowId(&ans[i].value[0], i);
		assignFiller(&ans[i].value[0], i);
	}
...
int teraWriteToFile(int fd, int  nrecs, terarec_t *recs){
	char buffer[RECS_IN_BUFFER * TERA_REC_WIDTH];
	int  nwritten = 0;
	...
	while(nwritten < nrecs){
		...
		while(bytes_written < goal_written){
			int retval = pwrite(fd, &buffer[bytes_written], goal_written - bytes_written, (nwritten) * TERA_REC_WIDTH + bytes_written);
			if(retval == -1){
				fprintf(stderr, "pwrite error in teraWriteToFile : %s\n", strerror(errno));
				return EXIT_FAILURE;
			}
